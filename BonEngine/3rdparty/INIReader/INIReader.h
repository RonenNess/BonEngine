...
public:
    ...
    // Return the list of keys found in section
	...
	// Get a string with direct pointer to string internal buffer
	const char* GetRef(std::string section, std::string name, const char* default_value) const;
	...
	// sets a value.
    // overridePrevVal - if true will replace previous value, if false will append with \n
	void SetValue(std::string section, std::string name, std::string value, bool overridePrevVal);

	// removes a key
	void RemoveKey(std::string section, std::string name);
...
protected:
    ...
    std::map<std::string, std::set<std::string> > _keys;
...
{
    return ((INIReader*)this)->_keys[section];
}
...
inline const char* INIReader::GetRef(std::string section, std::string name, const char* default_value) const
{
	std::string key = MakeKey(section, name);
                            ...
                            const char* value)
{
	...
	reader->SetValue(section, name, value, false);
    return 1;
}

void INIReader::SetValue(std::string section, std::string name, std::string value, bool overridePrevVal)
{
	std::string key = MakeKey(section, name);
	if (overridePrevVal) {
        _values[key] = value;
	...
	_keys[section].insert(name);
	_sections.insert(section);
}

void INIReader::RemoveKey(std::string section, std::string name)
{
	std::string key = MakeKey(section, name);
	_values.erase(key);
	_keys[section].erase(name);
}

bool INIReader::Save(const char* path) const
{
	FILE* file;
	...
	{
		fprintf_s(file, "\n[%s]\n", section.c_str());
		auto keys = Keys(section);
		for (auto key : keys)
		{
			const char* val = this->GetRef(section, key, nullptr);
			fprintf_s(file, "%s = %s\n", key.c_str(), val);
		}
	}
